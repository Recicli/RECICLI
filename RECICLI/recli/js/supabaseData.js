...
export async function fetchEmpresas(modulo = null) {
  let query = supabase.from("empresas").select("*, profiles(nombre, apellido, hojas)").order("created_at", { ascending: false });
...
export async function fetchProductos(options = {}) {
  let query = supabase.from("productos").select("*").eq("estado", "activo").order("created_at", { ascending: false });
